...
def get_order_code(id):
    order_no = str(time.strftime('%Y%m%d%H%M%S', time.localtime(time.time()))) + str(id)
    return order_no
...
class WxappOrder(http.Controller, BaseController):
    ...
    # 评价接口
    @http.route('/<string:sub_domain>/order/reputation', auth='public', methods=['POST', 'GET'], csrf=False, type='http')
    def reputation(self, sub_domain, **kwargs):
        try:
            ...
            if not saleOrder:
                return self.res_err(901)
            saleOrder.write({'customer_status': 'completed', 'reputation': 1, 'reputation_content': '非常愉快的一次购物！'})
